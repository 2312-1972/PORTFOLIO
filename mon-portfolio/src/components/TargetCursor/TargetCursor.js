...
const TargetCursor = ({ targetSelector = '.cursor-target', spinDuration = 2, hideDefaultCursor = true }) => {
  ...
  useEffect(() => {
    if (!cursorRef.current) return;
    // élimine le arrow pointer par juste le dot
    ...
    const enterHandler = e => {
      ...
      gsap.to(corners, { opacity: 1, duration: 0.3 });
      ...
      const leaveHandler = () => {
        ...
        if (cornersRef.current) {
          const corners = Array.from(cornersRef.current);
          gsap.to(corners, { opacity: 0, duration: 0.3 });
    ...
    return () => {
      ...
      //a réactiver pour le useeffect si besoin du dot à la place du arrow 
